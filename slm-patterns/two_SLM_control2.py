...
def centered_square(width, height, size, gray):
    ...
    y0 = (height - size) // 2; x0 = (width - size) // 2
...
def run():
    ...
    # ERIS: centered bright square, set once
    ew, eh = eris.width_px(), eris.height_px()
    show_pattern(eris, centered_square(ew, eh, SQUARE, ERIS_GRAY),
                 ERIS_USE_PHASE_DATA, TMP_ERIS, HEDSERR_NoError)
    print(f"  ERIS: centered {SQUARE}x{SQUARE} square held at gray {ERIS_GRAY}")

    PySpin, system, cams, cam = init_camera()
    print("  camera ready")

    # PLUTO: sweep the centered square
    pw, ph = pluto.width_px(), pluto.height_px()
    print(f"  sweeping PLUTO {SQUARE}x{SQUARE} square 0..255")
    for gray in range(NUM_GRAY_LEVELS):
        show_pattern(pluto, centered_square(pw, ph, SQUARE, gray),
