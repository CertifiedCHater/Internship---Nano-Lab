...
#
# """
# SLM Grayscale Response Measurement and Random Pattern Capture
#
# This script controls a HOLOEYE Spatial Light Modulator (SLM) and an optional
# PySpin-compatible camera to measure the optical response of the system.
#
# The script performs two main tasks:
#
# 1. Grayscale sweep:
#    - A half-screen grayscale pattern is displayed on the SLM.
#    - The gray value is swept from 0 to 255.
#    - For each gray level, a camera image is captured and saved.
#    - This measurement is typically used to characterize the intensity or
#      phase response of the SLM as a function of the applied gray value.
#
# 2. Random pattern projection:
#    - Random SLM patterns are generated and centered on the active SLM area.
#    - Each pattern is displayed and the corresponding camera image is captured.
#    - The displayed SLM pattern is also saved for reference.
#
# The script uses:
# - HOLOEYE SLM Display SDK (Python v4.0.0)
# - PySpin (FLIR/Spinnaker SDK) for camera control (optional)
# - OpenCV for image saving
#
# If PySpin is not available or no camera is detected, the script will run
# without camera acquisition.
#
# Outputs:
# - Captured camera images (.bmp)
# - Displayed SLM patterns (.bmp)
# Saved to the directory specified by OUTPUT_DIR.
#
# Author: Arthur Golovko & Jonas Getkate
# Context: SLM calibration / optical characterization
# """
#
# def center_pattern_sml(pattern, width=1920, height=1200):
#     """
#     Plaats een n×n patroon gecentreerd in een SLM van size height×width.
#
#     pattern: 2D array (n×n)
#     width:  totale breedte van de SLM (bijv. 1024)
#     height: totale hoogte van de SLM (bijv. 768)
#     """
#     n = pattern.shape[0]
#
#     # grote matrix die naar de SLM gestuurd wordt
